...
//  Y390:
//  store sampled audio in Swift array of floating-point values
//    here the array is initialized to empty:
...
class AudioController: NSObject, AURenderCallbackDelegate {
    ...
    {
        // Y390 testing 20170414:
        // print ("performRender is called \(self.counterOfCalls)")
        // self.counterOfCalls+=1
        // testing to see how many frames are read in, 512 print(inNumberFrames)
        ...
        if !audioChainIsBeingReconstructed {
            ...
            // filter out the higher frequencies of the signal
            _lowpass?.processInplace(ioPtr[0].mData!.assumingMemoryBound(to: Float32.self), numFrames: inNumberFrames)
...
//        }
    ...
    var lowPassFilterInstance: LowPassFilter {
        return _lowpass
    }
